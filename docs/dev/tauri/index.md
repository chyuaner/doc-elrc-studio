...
::: danger 本專案放棄支援！
本專案已經停止開發，因為在開發過程中遇到足以影響穩定性的大問題，後來就決定改採用Electron框架處理。
:::

請移駕electron-elrc-maker Repo。 雖然有實現原生GTK標題列按鈕與取代前端實作的標題列連動，但是Tauri實在太過精簡導致核心功能無法原生運作，只能用Workaround的方式繞，實測已經明顯出現穩定性問題，後來決定放棄此技術。 

---
::: warning 以下本文是由AI生成
是在功能製作過程中時順便備份內容當作參考用，我沒花心力為該頁手動校稿。
:::

本專案是基於 Tauri v2 + Next.js 的逐字歌詞製作工具，支援 Windows / Linux 桌面。
